...
export async function POST(req: NextRequest) {
  ...
  switch (action) {
    case 'pwa_settings_save':
    case 'all_save': {
      const reg = ((body.registration && typeof body.registration === 'object') ? body.registration : body) as Record<string, unknown>;
      const th = ((body.theme && typeof body.theme === 'object') ? body.theme : body) as Record<string, unknown>;
      const tb = ((body.table && typeof body.table === 'object') ? body.table : body) as Record<string, unknown>;
      patch = {
        registration: { enabled: !!reg.enabled, collectName: reg.collectName !== false },
        theme: { accent: th.accent ? String(th.accent) : null, logoUrl: isUrl(th.logoUrl) ? String(th.logoUrl).trim() : null, heroTagline: String(th.heroTagline ?? '').slice(0, 140) },
        table: { welcomePrefix: String(tb.welcomePrefix ?? 'Welcome to Table').slice(0, 40), allowManualPick: tb.allowManualPick !== false },
      };
      break;
    }
    ...
    case 'registration_save': {
      const reg = ((body.registration && typeof body.registration === 'object') ? body.registration : body) as Record<string, unknown>;
      patch = { registration: { enabled: !!reg.enabled, collectName: reg.collectName !== false } };
      break;
    }
    ...
    case 'theme_save': {
      const th = ((body.theme && typeof body.theme === 'object') ? body.theme : body) as Record<string, unknown>;
      patch = { theme: { accent: th.accent ? String(th.accent) : null, logoUrl: isUrl(th.logoUrl) ? String(th.logoUrl).trim() : null, heroTagline: String(th.heroTagline ?? '').slice(0, 140) } };
      break;
    }

    case 'table_save': {
      const tb = ((body.table && typeof body.table === 'object') ? body.table : body) as Record<string, unknown>;
      patch = { table: { welcomePrefix: String(tb.welcomePrefix ?? 'Welcome to Table').slice(0, 40), allowManualPick: tb.allowManualPick !== false } };
      break;
    }
