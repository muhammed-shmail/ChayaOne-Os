...
export default function DashboardClient({
  ...
  const [receiptForm, setReceiptForm] = useState({
    ...
    showTableNumber: outlet.receipt.showTableNumber,
    showOrderNumber: outlet.receipt.showOrderNumber,
    showDateTime: outlet.receipt.showDateTime,
    showItemNotes: outlet.receipt.showItemNotes,
    showTaxDetails: outlet.receipt.showTaxDetails,
    showDiscount: outlet.receipt.showDiscount,
    showUpiQr: outlet.receipt.showUpiQr,
    showScanAndPay: outlet.receipt.showScanAndPay,
    paperWidth: outlet.receipt.paperWidth,
    qrSize: outlet.receipt.qrSize,
  });
                ...
                <section className="card p-5">
                  ...
                  {tablesLoading && !tablesData ? (
                    ...
                    <div className="p-8 text-center bg-paper-3 rounded-xl border border-line">
                      ...
                      <p className="text-xs text-ink-3 mt-1">Select &ldquo;All&rdquo; or assign tables to this section in Settings.</p>
                    </div>
                  ) : (
                      ...
                      {shownGroups.map((g) => (
                        <div key={g.key} className="flex flex-col gap-2.5">
                          <div className="flex items-center justify-between border-b border-line pb-1.5">
                            <h5 className="font-bold text-xs uppercase tracking-wider text-ink-2">
                              {g.name}
                            </h5>
              ...
              return (
                <section className="card p-5">
                  ...
                  {tablesLoading && !tablesData ? (
                    ...
                    <p className="text-sm text-ink-3">No tables found in this section.</p>
                  ) : (
                        ...
                        <div key={g.key} className="flex flex-col gap-2.5">
                          {floorList.length > 0 && (
                            <div className="flex items-center gap-2 border-b border-line/60 pb-1.5">
                              <span className="text-xs font-extrabold uppercase tracking-wide text-ink-2">{g.name}</span>
                              <span className="text-[11px] text-ink-3">· {g.tables.length} table{g.tables.length > 1 ? 's' : ''}</span>
                            </div>
                          )}
